...
class GLApllet : public vl::Applet
{
public:
    GLApllet()
    {
    ...
    {
        innerTessLevel = (float) change;
        updateScene();
    }
    void changeOuterTesselation(int change)
    {
        outerTessLevel = (float) change;
        updateScene();
    }
    void changeDisplacement(int change)
    {
        dispLevel = (float) change;
        updateScene();
    }
    void initEvent()
  {
    ...
    tex_human->getTexParameter()->setMinFilter(vl::TPF_LINEAR);
...
*/
    ...
    glsl->gocUniform("innerTesselation")->setUniformF(5.0);
    glsl->gocUniform("outerTesselation")->setUniformF(8.0);
    glsl->gocUniform("dispLevel")->setUniformF(2.0);
   // glsl->gocUniform("tex_hm")->setUniformI(0);
    glsl->addAutoAttribLocation(0,"pos");
    glsl->activeAttribInfo("TexCoord");
  ...
  {
      glsl->gocUniform("innerTesselation")->setUniformF(innerTessLevel);
      glsl->gocUniform("outerTesselation")->setUniformF(outerTessLevel);
      glsl->gocUniform("dispLevel")->setUniformF(dispLevel);
  }
